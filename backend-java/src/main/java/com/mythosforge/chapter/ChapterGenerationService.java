...
public class ChapterGenerationService {
    ...
    /**
     * 组装 Writer LangGraph 入口 JSON（SSE 与 RabbitMQ Worker 共用）。
     */
    public ObjectNode buildWriterPayload(String projectId, int chapterNo, ChapterGenerateBody generateBody) {
        ...
        ChapterContractEntity chapterRow = chapterContractRepository
                ...
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "该章节章纲不存在"));
        ...
        JsonNode chapterJson = chapterRow.getRawJson();
        payload.set("storyContract", storyPayload);
        payload.set("chapterContract", chapterJson != null ? chapterJson : objectMapper.createObjectNode());
    ...
    public void requireProjectStoryAndChapter(String projectId, int chapterNo) {
        ...
        chapterContractRepository.findByStoryContractIdAndChapterNo(sid, chapterNo)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "该章节章纲不存在"));
    }
